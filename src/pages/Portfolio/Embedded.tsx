import Navigation from "@/components/Navigation";
import Footer from "@/components/Footer";
import { Link } from "react-router-dom";
import { ExternalLink, Calendar, Users, Star , Trash } from "lucide-react";
import { useState , useEffect } from "react";
import {useApi} from "@/hooks/useApi";
const Embedded = () => {
  const [editMode, setEditMode] = useState(false);
    type textContent = {
  _id: string; // Assuming _id is a string, adjust as necessary
  title: string;
  description: string;
  callToActionTitle: string;
  callToActionDesc: string;
  startProjectButton: string;
  caseStudyButton: string;
  };



  const [textContent, setTextContent] = useState<textContent>({
    _id: "",
    title: "",
    description: "",
    callToActionTitle: "",
    callToActionDesc: "",
    startProjectButton: "",
    caseStudyButton: "",
  });

  type Project = {
    id: number;
    name: string;
    category: string;
    tags: string[];
    image: string;
    summary: string;
    year: string;
    client: string;
    rating: number;
  };

  const [projects, setProjects] = useState<Project[]>([]);

  const {
   createData: createProject,
  updateData: updateProject,
  deleteData: deleteProject,
  fetchData: fetchProjects,
  loading: loadingProjects,
  data: projectData,
  } = useApi('projects');

  const {
     deleteData:deleteProjectDetails
  } = useApi("projectDetails")

const {
  fetchData: fetchPortfolio,
  loading: loadingPortfolio,
  data: portfolioData,
  createData: createPortfolio,
  updateData: updatePortfolio,
 } = useApi('portfolio');

useEffect(() => {
  const loadProjects = async () => {
    await fetchProjects(); // fetch from /api/projects
    await fetchPortfolio(); 
  };
  loadProjects();
}, []);

useEffect(() => {
   if (projectData) {
    console.log("Projects fetched:", projectData); 
    setProjects(projectData as Project[]); // if you have a `setProjects` state
  }   
}, [projectData]);


useEffect(() => {
  if (portfolioData) {
    console.log("Portfolio fetched:", portfolioData);
    setTextContent(portfolioData as typeof textContent); // if you have a `setPortfolio` state
  }
}, [portfolioData]);


 const editModeHandler = async () => {
  if (editMode) {
    try {
      // ✅ Update portfolio content
      if (textContent._id) {
    updatePortfolio(textContent._id, textContent)
      .then(() => console.log("Portfolio updated successfully"))
      .catch((error) => console.error("Error updating portfolio:", error));
  }

      for (const project of projects) {
        if (project.id) {
          // Fire and forget
          updateProject(project.id.toString(), project)
            .then(() => console.log(`Updated project ${project.id}`))
            .catch((err) => console.error(`Failed to update project ${project.id}`, err));
        }
      }
      console.log("All projects updated successfully");
    } catch (error) {
      console.error("Error updating content:", error);
    }
  }

  // ✅ Toggle edit mode
  setEditMode((prev) => !prev);
};




  const [showForm, setShowForm] = useState(false);
  const [newProject, setNewProject] = useState({
    name: "",
    category: "",
    tags: "",
    image: "",
    summary: "",
    year: "",
    client: "",
    rating: 5,
  });

  const handleAddProject = () => {

  const tags = newProject.tags.split(",").map((tag) => tag.trim());

  // Find the max existing ID
  const maxId = projects.length > 0 ? Math.max(...projects.map(p => p.id)) : 0;

  const newEntry = {
    ...newProject,
    id: maxId + 1, // 👈 increment by 1
    tags,
  };

  setProjects([newEntry, ...projects]);
  createProject(newEntry)
    .then(() => console.log("Project created successfully"))
    .catch((error) => console.error("Error creating project:", error));
  setNewProject({
    name: "",
    category: "",
    tags: "",
    image: "",
    summary: "",
    year: "",
    client: "",
    rating: 5,
  });
  setShowForm(false);
};


  const categories = [
    "All",
    "E-commerce",
    "Healthcare",
    "FinTech",
    "Education",
    "Real Estate",
    "Food & Beverage",
    "Government",
    "Marketing",
    "Gaming",
  ];

  return (
    <div className="min-h-screen bg-hero-gradient font-inter text-white overflow-hidden relative">
      <div className="absolute top-64 right-0 w-[600px] h-[653px] rounded-full bg-gradient-to-b from-[rgba(84,84,212,0.20)] to-[rgba(84,84,212,0.08)] blur-[150px]" />
      <div className="absolute top-10 left-96 w-[467px] h-[1399px] rounded-full bg-gradient-to-b from-[rgba(251,168,28,0.11)] to-[rgba(224,86,136,0.06)] blur-[150px] rotate-[65.712deg]" />
      <div className="absolute -top-16 -left-80 w-[467px] h-[1234px] rounded-full bg-gradient-to-b from-[rgba(84,84,212,0.27)] to-[rgba(84,84,212,0.11)] blur-[150px] rotate-[-54.374deg]" />

      <Navigation />

      <div className="flex justify-end px-4 mt-4">
        <button
          onClick={editModeHandler}
          className="bg-brand-teal px-4 py-2 rounded text-white font-semibold"
        >
          {editMode ? "View Mode" : "Edit Mode"}
        </button>
      </div>

      <section className="relative z-10 py-20 px-4">
        <div className="max-w-6xl mx-auto text-center">
          {editMode   ? (
              <input
               type="text"
               className="text-5xl lg:text-7xl font-bold font-inter mb-6 w-full text-center outline-none border-b border-white bg-transparent text-white"
               value={textContent.title}
               onChange={(e) =>
              setTextContent({ ...textContent, title: e.target.value })
                         }
                />
              ) : (
        <h1 className="text-5xl lg:text-7xl font-bold font-inter mb-6 bg-hero-text-gradient bg-clip-text text-transparent text-center">
               {textContent.title}
                </h1>
              )}


          <p className="text-xl text-[#D2D0DD] max-w-4xl mx-auto font-inter leading-relaxed mb-12">
            {editMode  ? (
              <textarea
                rows={3}
                className="bg-transparent border border-white rounded p-2 w-full text-center resize-none"
                value={textContent.description}
                onChange={(e) =>
                  setTextContent({ ...textContent, description: e.target.value })
                }
              />
            ) : (
              textContent.description
            )}
          </p>

          <div className="flex flex-wrap justify-center gap-3 mb-8">
            {categories.map((category, index) => (
              <button
                key={index}
                className={`px-6 py-2 rounded-full border border-glass-border backdrop-blur-sm transition-all duration-300 ${
                  index === 0
                    ? "bg-brand-teal text-white"
                    : "bg-glass-white text-[#D2D0DD] hover:bg-brand-teal hover:text-white"
                }`}
              >
                {category}
              </button>
            ))}
          </div>

          <button
            onClick={() => setShowForm(true)}
            className="bg-brand-teal px-8 py-3 rounded-29 text-white font-inter text-lg shadow-glow hover:scale-105 transition-transform"
          >
            + Add New Project
          </button>
        </div>
      </section>

    

    {loadingProjects && (!projects || projects.length === 0)  ? (
  <div className="col-span-full flex justify-center py-12">
    <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-white"></div>
  </div>
) : (
  <section className="relative z-10 py-12 px-4">
    <div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {(projects).map((project, index) => {
          if (!project) return null; // safeguard
          return (
            <div
              key={project.id}
              className="group bg-glass-white backdrop-blur-sm border border-glass-border rounded-[24px] overflow-hidden hover:scale-105 hover:shadow-2xl transition-all duration-500 cursor-pointer"
              style={{
                animationDelay: `${index * 0.1}s`,
                animation: "fadeInScale 0.6s ease-out forwards",
              }}
            >
              <div className="relative h-48 overflow-hidden">
                      {editMode ? (
                        <div className="w-full h-48 relative flex flex-col justify-center items-center">
                          <img
                            src={project.image}
                            alt="preview"
                            className="absolute inset-0 w-full h-full object-cover opacity-30 pointer-events-none"
                          />
                          <input
                            type="text"
                            value={project.image}
                            onChange={(e) => {
                              const updated = [...projects];
                              updated[index].image = e.target.value;
                              setProjects(updated);
                            }}
                            className="relative z-10 w-4/5 bg-gray-900/70 text-white p-2 text-xs rounded border border-white focus:outline-none"
                            placeholder="Paste image URL here"
                          />
                        </div>
                      ) : (
                        <div className="w-full h-48">
                          <img
                            src={project.image}
                            alt={project.name}
                            className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                          />
                        </div>
                      )}








                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                <div className="absolute top-4 right-4 flex items-center gap-2">
                  <span className="bg-brand-teal px-3 py-1 rounded-full text-xs font-semibold">
                    {editMode ? (
                      <input
                        type="text"
                        value={project.category}
                        onChange={(e) => {
                          const updated = [...projects];
                          updated[index].category = e.target.value;
                          setProjects(updated);
                        }}
                        className="bg-transparent border-b border-white text-white text-xs w-20"
                      />
                    ) : (
                      project.category
                    )}
                  </span>
                  {editMode && (
       
          <Trash
  className="w-4 h-4 text-white hover:text-red-500 cursor-pointer"
  onClick={() => {
    const stringId = project.id.toString(); // Convert project.id to string

    // Optimistically update the local state
    setProjects(projects.filter((p) => p.id !== project.id));

    // Call your API functions with stringId
    deleteProject(stringId)
      .then(() => {
        console.log("Project deleted successfully");
      })
      .catch((err) => {
        console.error("Error deleting project:", err);
      });

    deleteProjectDetails(stringId)
      .then(() => {
        console.log("Project details deleted successfully");
      })
      .catch((err) => {
        console.error("Error deleting project details:", err);
      });
  }}
/>



)}
                </div>
                <div className="absolute bottom-4 left-4 flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${
                        i < project.rating
                          ? "text-yellow-400 fill-current"
                          : "text-gray-400"
                      }`}
                    />
                  ))}
                </div>
              </div>

              <div className="p-6">
                <h3 className="text-xl font-bold text-white mb-2 font-inter group-hover:text-brand-teal transition-colors duration-300">
                  {editMode ? (
                    <input
                      type="text"
                      value={project.name}
                      onChange={(e) => {
                        const updated = [...projects];
                        updated[index].name = e.target.value;
                        setProjects(updated);
                      }}
                      className="bg-transparent border-b border-white text-white w-full"
                    />
                  ) : (
                    project.name
                  )}
                </h3>

                <div className="flex flex-wrap gap-2 mb-4">
                  {editMode
                    ? project.tags.map((tag, tagIndex) => (
                        <input
                          key={tagIndex}
                          type="text"
                          value={tag}
                          onChange={(e) => {
                            const updated = [...projects];
                            updated[index].tags[tagIndex] = e.target.value;
                            setProjects(updated);
                          }}
                          className="px-3 py-1 bg-gray-800/50 text-xs text-[#D2D0DD] rounded-full border border-gray-700 w-20"
                        />
                      ))
                    : project.tags.map((tag, tagIndex) => (
                        <span
                          key={tagIndex}
                          className="px-3 py-1 bg-gray-800/50 text-xs text-[#D2D0DD] rounded-full border border-gray-700"
                        >
                          {tag}
                        </span>
                      ))}
                </div>

                <p className="text-[#D2D0DD] text-sm font-inter leading-relaxed mb-4 line-clamp-3">
                  {editMode ? (
                    <textarea
                      rows={2}
                      className="w-full bg-transparent border border-white text-white p-1 rounded resize-none"
                      value={project.summary}
                      onChange={(e) => {
                        const updated = [...projects];
                        updated[index].summary = e.target.value;
                        setProjects(updated);
                      }}
                    />
                  ) : (
                    project.summary
                  )}
                </p>

                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 text-xs text-[#D2D0DD]">
                    <div className="flex items-center gap-1">
                      <Calendar className="w-4 h-4" />
                      {editMode ? (
                        <input
                          type="text"
                          value={project.year}
                          onChange={(e) => {
                            const updated = [...projects];
                            updated[index].year = e.target.value;
                            setProjects(updated);
                          }}
                          className="bg-transparent border-b border-white text-white w-12"
                        />
                      ) : (
                        project.year
                      )}
                    </div>
                    <div className="flex items-center gap-1">
                      <Users className="w-4 h-4" />
                      {editMode ? (
                        <input
                          type="text"
                          value={project.client}
                          onChange={(e) => {
                            const updated = [...projects];
                            updated[index].client = e.target.value;
                            setProjects(updated);
                          }}
                          className="bg-transparent border-b border-white text-white w-20"
                        />
                      ) : (
                        project.client
                      )}
                    </div>
                  </div>
                  <Link
                    to={`/portfolio/${project.id}`}
                    className="flex items-center gap-1 text-brand-teal hover:text-white transition-colors duration-300 opacity-0 group-hover:opacity-100"
                  >
                    <span className="text-sm font-semibold">View More</span>
                    <ExternalLink className="w-4 h-4" />
                  </Link>
                </div>
              </div>
            </div>
          );
        })}
      </div>
    </div>
  </section>
)}





      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/70 flex justify-center items-center">
          <div className="bg-gray-900 p-8 rounded-xl max-w-md w-full">
            <h2 className="text-2xl font-bold mb-4 text-white">Add New Project</h2>
            {Object.keys(newProject).filter(k => k !== 'tags' && k !== 'rating').map((key) => (
              <input
                key={key}
                type="text"
                placeholder={key.charAt(0).toUpperCase() + key.slice(1)}
                className="w-full bg-gray-800 border border-gray-700 p-2 rounded mb-3 text-white"
                value={newProject[key]}
                onChange={(e) =>
                  setNewProject({ ...newProject, [key]: e.target.value })
                }
              />
            ))}
            <input
              type="text"
              placeholder="Tags (comma-separated)"
              className="w-full bg-gray-800 border border-gray-700 p-2 rounded mb-3 text-white"
              value={newProject.tags}
              onChange={(e) =>
                setNewProject({ ...newProject, tags: e.target.value })
              }
            />
          <div className="flex justify-between  mt-4">
            <button
              onClick={handleAddProject}
              className="bg-brand-teal px-6 py-2 rounded font-bold hover:bg-white hover:text-gray-900 transition-all"
            >
              Save Project
            </button>
            <button
              onClick={() => {
                setShowForm(false);
                setNewProject({
                  name: "",
                  category: "",
                  tags: "",
                  image: "",
                  summary: "",
                  year: "",
                  client: "",
                  rating: 5,
                });
              }}
              className="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400 transition"
            >
              Cancel
            </button>
            </div>
          </div>
        </div>
      )}

      <section className="relative z-10 py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl lg:text-5xl font-bold text-white mb-6 font-inter">
            {editMode ? (
              <input
                type="text"
                className="bg-transparent border-b border-white outline-none w-full text-center"
                value={textContent.callToActionTitle}
                onChange={(e) =>
                  setTextContent({
                    ...textContent,
                    callToActionTitle: e.target.value,
                  })
                }
              />
            ) : (
              textContent.callToActionTitle
            )}
          </h2>
          <p className="text-xl text-[#D2D0DD] mb-8 font-inter">
            {editMode  ? (
              <textarea
                rows={2}
                className="bg-transparent border border-white rounded p-2 w-full text-center resize-none"
                value={textContent.callToActionDesc}
                onChange={(e) =>
                  setTextContent({
                    ...textContent,
                    callToActionDesc: e.target.value,
                  })
                }
              />
            ) : (
              textContent.callToActionDesc
            )}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {editMode  ? (
              <>
                <input
                  type="text"
                  className="bg-gray-800 border border-gray-600 px-4 py-2 rounded text-white"
                  value={textContent.startProjectButton}
                  onChange={(e) =>
                    setTextContent({
                      ...textContent,
                      startProjectButton: e.target.value,
                    })
                  }
                />
                <input
                  type="text"
                  className="bg-gray-800 border border-gray-600 px-4 py-2 rounded text-white"
                  value={textContent.caseStudyButton}
                  onChange={(e) =>
                    setTextContent({
                      ...textContent,
                      caseStudyButton: e.target.value,
                    })
                  }
                />
              </>
            ) : (
              <>
                <button className="bg-button-gradient px-8 py-4 rounded-29 text-white font-inter text-lg shadow-glow hover:scale-105 transition-transform">
                  {textContent.startProjectButton}
                </button>
                <button className="border-2 border-white text-white px-8 py-4 rounded-29 font-inter text-lg hover:bg-white hover:text-gray-900 transition-colors">
                  {textContent.caseStudyButton}
                </button>
              </>
            )}
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

const style = document.createElement("style");
style.textContent = `
  @keyframes fadeInScale {
    from {
      opacity: 0;
      transform: translateY(30px) scale(0.95);
    }
    to {
      opacity: 1;
      transform: translateY(0) scale(1);
    }
  }
  .line-clamp-3 {
    display: -webkit-box;
    -webkit-line-clamp: 3;
    -webkit-box-orient: vertical;
    overflow: hidden;
  }
`;
document.head.appendChild(style);

export default Embedded;
